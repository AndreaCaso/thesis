...
for cc1 = 1:length(beta_str_var)
    for cc2 = 1:length(alpha_str_var)
        ...
        actMean(cc1,cc2) = mean(o_SMAtot(o_SMAtot > 0.1));
        actMedian(cc1,cc2) = median(o_SMAtot(o_SMAtot > 0.1));
...
plot(beta_str_var,actMean(:,subPlotNum)');
hold on; 
plot(beta_str_var,actMedian(:,subPlotNum)');
