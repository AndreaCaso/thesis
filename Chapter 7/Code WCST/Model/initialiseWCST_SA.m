...
%extPFC = 1; %controls the external to the cog schema: Last 0.8
extNoise = 0.25; % controls external (uniform noise), to PFC and to SMA
...
pfc.pfcNoise = 0.25*unifrnd(-1,1,1,lstim+1);
sma.smaNoise = 0.25*unifrnd(-1,1,4,timeSteps);

pfc.pred_act = zeros(lstim+1,3);
pfc.entropyPFC = zeros(lstim+1,3);

reward.reward_value = nan(lstim+1,3);
reward.TDV = zeros(lstim+1,3);
%reward.discount_TDV = 0.00; %gamma value in temporal difference learning

%epsilon.eps_str = 0.5; % learning rate for STRIATUM 
%epsilon.eps_pfc = 0.25; % learning rate for PFC 
