...
extPFC = unifrnd(0.8,1); %down this and up PRSRE
extNoise = 0.10; % controls external (uniform noise), to PFC and to SMA
...
pfc.pfcNoise = 0.10*randn(1,num_trials);
sma.smaNoise = 0.10*randn(num_trials,timeSteps);
...
reward.discount_TDV = unifrnd(0,0.25); 

epsilon.eps_str = []; % learning rate for STRIATUM 
epsilon.eps_pfc = []; % learning rate for PFC  
...
satfnc.alpha_str_pfc = unifrnd(10,15)*(ones(1,num_rules)); 
satfnc.alpha_str_sma = 10*(ones(1,num_circles)); 

satfnc.beta_thal = unifrnd(0.3,0.5); %increasing it doesn't change PRU much

satfnc.beta_sma = 0.40*ones(1,num_circles); 
satfnc.beta_pfc = 0.40*ones(1,num_rules);
...
weights.w_RULE_PRES = unifrnd(0.4,0.5);  % this parameter is essential
