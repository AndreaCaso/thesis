...
function subj_sim_wcst = runWCSTvar_Set(subjects_tot,timed_var,varargin)
...
for subject_num = 1:subjects_tot
 ...
 subj_sim_wcst(subject_num).TE = sum(performances.TE) - sum(performances.MISS);
 ...
 subj_sim_wcst(subject_num).MISS = sum(performances.MISS);
