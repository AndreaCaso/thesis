...
timeLength = ceil(3000/48*lstim);
timeSteps = round(timeLength/f_sampl) + 1;
 
extPFC = 1.30; %controls the external to the cog schema: Last 0.8
extNoise = 0.30; % controls external noise, to PFC and to SMA
...
var_dynamic = 7e3;
thresholds.dynamic = normrnd(5e4,var_dynamic,1,timeSteps);  %5e4*ones(1,timeSteps) + 
thresholds.static = 0.5;
if timed == true
    thresholds.miss = 480;  %480 gets 1% MISS
elseif timed == false
    thresholds.miss = Inf; 
end

pfc.pfcNoise = unifrnd(-0.25,0.25,1,lstim+1);
%0.25*unifrnd(-1,1,1,lstim+1);
%sma.smaNoise = normrnd(0.25,0.25,1,lstim+1);
%0.25*unifrnd(-1,1,4,timeSteps);
...
epsilon.eps_str = 0.30; % learning rate for STRIATUM 
epsilon.eps_pfc = 0.15; % learning rate for PFC 
...
weights.w_RULE_PRES = 1.00; %1.2792;   % this parameter is essential
...
o_EXT_SMA = 0 + unifrnd(-extNoise,extNoise,4,timeSteps);
%extNoise*ones(4, timeSteps) + sma.smaNoise;

counter.tt_trial_num = zeros(1,lstim);
category = ['c' 's' 'n' 'c' 's' 'n' 'c' 's' 'n' 'c' 's' 'n' 'c' 's' 'n' 'c' 's' 'n' 'c' 's' 'n' 'c' 's' 'n' 'c' 's' 'n' 'c' 's' 'n' 'c' 's' 'n' 'c' 's' 'n']; % Colour - Form - Number - Colour - Form - Number
...
performances.MISS = zeros(1,lstim);
