...
if counter.trial_num_time > thresholds.miss &&  any(counter.area_sma < mean(thresholds.dynamic))
    ...
    performances.MISS(counter.trial_num) = 1;
...
if any(counter.area_sma > thresholds.dynamic(tt)) && any(counter.sma_value > thresholds.static)
...
if performances.CC < ceil(lstim/counter.criterion) && counter.trial_num >= counter.criterion
    if counter.correct_counter == counter.criterion
